...
(function () {
  ...
  function frameScreenHook(ctx) {
    ...
    // HUD sits BELOW the hole-start title card (top-left, ~y20-145) so it never overlaps it.
    if (hint > 0) {
      ctx.save();
      ctx.globalAlpha = Math.min(1, hint / 90);
      ctx.fillStyle = 'rgba(8,12,20,0.62)'; ctx.fillRect(20, 152, 520, 76);
      ctx.fillStyle = 'rgba(235,244,255,0.95)'; ctx.font = "13px 'Departure Mono', monospace"; ctx.textAlign = 'left';
      ctx.fillText('PARTICLE FLUID (PBF) · click / HOLD above the ground → POUR', 32, 174);
      ctx.fillText('fills the narrow notch + UNDER the overhang, self-levels', 32, 194);
      ctx.fillText('drag back from the ball → aim + power, release → SHOOT', 32, 214);
      ctx.restore();
      hint--;
    }
    ...
    ctx.fillText(P.length + ' particles · ' + Math.round(fps) + ' fps', 32, hint > 0 ? 242 : 152);
    ctx.restore();
    // CLEAR button
    var b = clearBtnRect();
    ctx.save();
