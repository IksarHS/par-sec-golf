...
function gameLoop(ts) {
  ...
  if (window.RG_DEV && typeof aiEnabled !== 'undefined' && aiEnabled) {   // autoplay is DEV-ONLY (player build never drives the bot, even if aiEnabled is forced)
...
function _startBackgroundLoop() {
  ...
  _backgroundInterval = setInterval(() => {
    if (!window.RG_DEV || typeof aiEnabled === 'undefined' || !aiEnabled) return;   // autoplay is DEV-ONLY
    _ballLogFrame++;
