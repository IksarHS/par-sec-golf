// ── The Scorecard / "collection" store ─────────────────────
// PC = single-player ADVENTURE: there is NO money/shop/ship economy (that was the mobile
// roguelike, stripped 2026-06-29). What remains here is the per-slot SCORE record + the recap
// "collection" art that reads it. Every surface course has nine hole SLOTS (terrain is seeded
// fresh each run; identity lives in the slot: earth hole 3, moon hole 7). Each slot remembers
// your BEST TIER:
//
//   tier 1  completed
//   tier 2  at par
//   tier 3  under par
//   tier 4  hole-in-one (an "ace" you hold)
//
// settleRun() (called by wrap.onTransitionEnd) persists each slot's best tier; nothing is paid.
// The rest of the file renders that tier store as the recap collection art — the scorecard grid,
// the constellation sky, the journey path, and the clubhouse paper card.
...
(function () {
  ...
  window.RG_ECON = {
    ...
    // (Removed 2026-06-29: drawTakeLine — the +$ money take. PC adventure has no money; the recap
    //  in wrap.js no longer references it.)
  };
})();
