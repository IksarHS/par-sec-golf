...
//   1  Wayside Cache  (MOOD)    a tiny half-buried crate resting on the ground by the
//                               tee. A purely cosmetic find — glyph-only, no card, grants nothing.
...
//                               long hole's number. Buys KNOWLEDGE, grants nothing.
...
//
// DESIGN LAW (CLAUDE.md): finds buy FACTS and ACCESS, not putting power. The four registers
// span cosmetic -> knowledge -> place WITHOUT ever touching a shot:
//   cache = cosmetic (grants nothing), sign = a FACT (you could have read it),
...
//
// Peel-off: delete this file + its <script> in devbuild.html + the two RG_EVENT lines in
...
(function () {
  ...
  // ── Grants (knowledge / place only — never shot power) ────────────
  ...
  // strictly a fact you COULD have learned yourself (the longest hole) — never predictive of
  // RNG you couldn't otherwise know.
  function scoutFact() {
  ...
  function maybeArm(holeIdx, ignoreOdds) {
    ...
    ev = null;
    if (!v) return false;
    if (!(window.RG && RG.active)) return false;
  ...
  function resolve() {
    ...
    if (ev.kind === 'shrine') unsealWaypoint();
  ...
  function drawSign(ctx, x, y, a) {
    ...
    if (ev.resolved && f) {
      const line = (f.kind === 'long') ? ('hole ' + (f.hole + 1) + ' is the long one') : '';
      if (line) {
