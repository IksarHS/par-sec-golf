...
// when aiEnabled, then draw(), so travel/transition animations still play). Plays real runs, so
// progression (stars, per-planet scores) actually fires. Also the headless-verify
// harness:
//
...
(function () {
  ...
  window.aiUpdate = function () {
    ...
    if (state === STATE_COMPLETE) {
      ...
      if (botCompleteT > 120) {                 // hold the recap ~2s at 1x so it's readable
        ...
        // The whole-game tour: a Moon recap heads home to Earth; otherwise a fresh run on the
        // current world. (Surface courses normally auto-travel onward in wrap.onTransitionEnd
        // before the recap even holds, so this is mostly the loop's New Run fallback.)
  ...
  // gated on window.RG_DEV (set only by devbuild.html). In the player build RG_DEV is unset → autoplay is inert.
