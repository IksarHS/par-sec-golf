...
(function () {
  ...
  function _paintBand(cond, mat, span) {
    ...
    for (let k = 0; k < vertices.length; k++) {
      ...
      if (v.mat === 'water') continue;   // never overwrite a hazard
  ...
  const RG = {
    ...
    _clampTerrainMats() {
      ...
      for (let i = 0; i < vertices.length; i++) {
        ...
        const m = vertices[i].mat;
        if (m === 'water' || m === 'ice' || m === 'rock' || m === 'mud') continue;   // hazards / condition-band paints
    ...
    _armCrane() {
      ...
      if (install) install();                                     // loads + sinks the destination; sets state/camera.y=depth
      ...
      // Restore the departure framing the install clobbered (it set camera.y=±depth).
  ...
  // (Removed 2026-06-30: the rgSecretPointer capture-phase click/contextmenu/touch router — it gated on
  //  window.RG_runSecretHook, which the removed secrets subsystem never defines now, so it was inert.)
  ...
  window.initFirebase = function () {
    ...
    // DEFAULT physics = REALISTIC (the lab's realistic preset). Captures the SHIP baseline first, so the
    // dev F-toggle (physics-toggle.js) can still switch back. Applied to PRISTINE before the first run so
    // it holds across every hole + travel, in BOTH the dev build and the bundled player build.
    if (RG.setPhysicsProfile) RG.setPhysicsProfile('realistic');
