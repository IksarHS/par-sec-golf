...
// per-slot best tiers, per-planet scores, the username, knowledge flags, waypoints, the
// constellation count, the collection. They're written from a handful of files
// (economy.js, profile.js, run.js, event.js, wrap.js). This module
...
// rg-best-<seed>, rg-knows-<flag>, rg-waypoint-<course>) and any future
...
//
// Note: `dg-seed` is an ENGINE key, owned by main.js/the core, not the
// roguelike layer — it's the current terrain seed, not progress. A reset leaves it be;
...
(function () {
  ...
  window.RG_SAVE = {
    ...
    //   { app:'space-golf', v:<VERSION>, ts:<ms>, keys:{ 'rg-tier-moon-3':'4', ... } }
    ...
    // Wipe all `rg-` keys → a virgin first-boot (no scores, no tiers, no knowledge, nothing
    // collected). The designer uses this to re-experience the cold open from the first hole.
