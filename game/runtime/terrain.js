...
//                        ridge to crest, a step down). The NOTCH shapes (cliff_shelf,
//                        shelf_drop_shelf) are deliberately excluded — those bury the cup in a
//                        slot-pit (arcade, not crafted).
//
...
(function () {
  ...
  // here so a signature hole is never a no-op. Still no notch shapes (cliff_shelf, etc.).
  const CRAFTED = ['mesa', 'shelf'];
  // Bold landforms — a single confident climb/drop per hole. Still no notch shapes.
  ...
  const SURFACE = { 'earth-course': 1, 'run-course': 1, 'moon': 1 };   // the surface courses (only these get treatments)
  let name = 'gentle';
