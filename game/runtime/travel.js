...
(function () {
  ...
  // ── Variant 2: the tiny ball crossing ────────────────────────────────────────
  // A small ball coasts across the EMPTY upper sky band with a faint contrail and a single
  // tinted result dot in tow — the "crossing the friendly dark" motif the vision names. A gentle
  // vertical bob makes it read as coasting, not on rails. Result is carried by the trailing dot's
  // COLOUR (no ambiguous "E" text chip floating in the sky).
  function drawCrossing(ctx, info, t) {
    ...
    // the craft: a clean little ball coasting across the sky
    ctx.translate(x, y);
