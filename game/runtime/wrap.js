...
(function () {
  ...
  MODE = Object.assign({}, base, {
    ...
    onTransitionEnd() {
      ...
      if (typeof state !== 'undefined' && state === STATE_COMPLETE && window.RG && RG.active) {
        ...
        if (window.RG_SCORES && RG._surfaceRunOnly && RG._surfaceRunOnly()) {
    ...
    // Camera override hook: an atlas planet (or the portrait static cam) can take over framing;
    // otherwise the engine's per-hole fixed camera stands. No-op for the base surface game.
    updateCamera() {
  ...
  // The completion buttons: a single primary New Run (plus a small blue Earth home glyph on the Moon).
  function drawCompleteButtons(y, fade) {
  ...
  // New Run (and the Earth home glyph on the Moon) — are reachable on a phone, where the
  // engine's touchstart preventDefault swallows the synthetic mouse event the layer used to rely on.
