...
// Mono. Reads REAL progress from RG_SCORES. Owns this screen only; never touches devbuild.html / the base game.
(function () {
'use strict';
...
function draw(){ hits=[]; drawSky(); ctx.textBaseline='alphabetic';
  ...
  drawTravel(); }
function loop(){ frame++;
  if (traveling){ traveling.t+=0.018; if (traveling.t>=1.12){ var base; try{ var tl=(window.top&&window.top!==window.self)?window.top.location:location; base=tl.href.split('#')[0].split('?')[0]; }catch(e){ base='devbuild.html'; }
